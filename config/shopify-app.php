...
return [
    ...
    */
    ...
    'webhooks' => [
        // App uninstall — clears shop token (handled by AppUninstalledJob)
        [
        ...
        [
            'topic'   => env('SHOPIFY_WEBHOOK_3_TOPIC', 'ORDERS_UPDATED'),
            'address' => env('SHOPIFY_WEBHOOK_3_ADDRESS', env('APP_URL') . '/webhook/orders-updated'),
        ],
        [
            'topic'   => env('SHOPIFY_WEBHOOK_4_TOPIC', 'ORDERS_PAID'),
            'address' => env('SHOPIFY_WEBHOOK_4_ADDRESS', env('APP_URL') . '/webhook/orders-paid'),
        ],
        [
            'topic'   => env('SHOPIFY_WEBHOOK_5_TOPIC', 'ORDERS_FULFILLED'),
            'address' => env('SHOPIFY_WEBHOOK_5_ADDRESS', env('APP_URL') . '/webhook/orders-fulfilled'),
        ],
        [
            'topic'   => env('SHOPIFY_WEBHOOK_6_TOPIC', 'ORDERS_CANCELLED'),
            'address' => env('SHOPIFY_WEBHOOK_6_ADDRESS', env('APP_URL') . '/webhook/orders-cancelled'),
        ],
        [
            'topic'   => env('SHOPIFY_WEBHOOK_7_TOPIC', 'ORDERS_DELETE'),
            'address' => env('SHOPIFY_WEBHOOK_7_ADDRESS', env('APP_URL') . '/webhook/orders-delete'),
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | Shopify Webhook Jobs
    |--------------------------------------------------------------------------
    |
    | Maps each webhook topic to a dedicated job class.
    | Kyon147/Osiset dispatches the mapped job automatically when a webhook
    | is received, passing ($shopDomain, $data) to the constructor.
    |
    */

    'webhook_jobs' => [
        'app/uninstalled'  => \App\Jobs\AppUninstalledJob::class,
        'orders/create'    => \App\Jobs\OrdersCreateJob::class,
        'orders/updated'   => \App\Jobs\OrdersUpdatedJob::class,
        'orders/paid'      => \App\Jobs\OrdersPaidJob::class,
        'orders/fulfilled' => \App\Jobs\OrdersFulfilledJob::class,
        'orders/cancelled' => \App\Jobs\OrdersCancelledJob::class,
        'orders/delete'    => \App\Jobs\OrdersDeleteJob::class,
    ],
    ...
    */
    ...
    'after_authenticate_job' => [
        // AfterAuthSyncJob is an adapter: the package always passes the full
        // shop/User model here, but SyncShopifyOrdersJob requires int $userId.
        // AfterAuthSyncJob extracts the ID and delegates correctly.
        [
            'job'    => \App\Jobs\AfterAuthSyncJob::class,
            'inline' => false, // Dispatch to queue (non-blocking)
        ],
    ],
