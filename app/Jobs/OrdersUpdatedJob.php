 ...
 * OrdersUpdatedJob
 *
 * Handles the orders/updated Shopify webhook.
 */
class OrdersUpdatedJob implements ShouldQueue
{
    ...
    public function handle(IShopQuery $shopQuery): void
    {
        ...
        if (! $user) {
            Log::error('[OrdersUpdatedJob] Shop user not found', [
        ...
        try {
            ...
            Log::info('[OrdersUpdatedJob] Completed successfully', [
                'shop'  => $shopDomainNative,
                'topic' => 'orders/updated',
            ]);
        } catch (Throwable $e) {
            Log::error('[OrdersUpdatedJob] Failed', [
                'shop'  => $shopDomainNative,
                'error' => $e->getMessage(),
            ]);
            throw $e;
    ...
    public function failed(Throwable $e): void
    {
        ...
        Log::error('[OrdersUpdatedJob] Permanently failed after all retries', [
            'shop'  => $domain,
            'error' => $e->getMessage(),
        ]);
    }
