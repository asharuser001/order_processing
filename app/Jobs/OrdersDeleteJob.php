 ...
 * OrdersDeleteJob
 *
 * Handles the orders/delete Shopify webhook.
 */
class OrdersDeleteJob implements ShouldQueue
{
    ...
    public function handle(IShopQuery $shopQuery): void
    {
        ...
        if (! $user) {
            Log::error('[OrdersDeleteJob] Shop user not found', [
        ...
        try {
            ...
            Log::info('[OrdersDeleteJob] Completed successfully', [
                'shop'  => $shopDomainNative,
                'topic' => 'orders/delete',
            ]);
        } catch (Throwable $e) {
            Log::error('[OrdersDeleteJob] Failed', [
                'shop'  => $shopDomainNative,
                'error' => $e->getMessage(),
            ]);
            throw $e;
    ...
    public function failed(Throwable $e): void
    {
        ...
        Log::error('[OrdersDeleteJob] Permanently failed after all retries', [
            'shop'  => $domain,
            'error' => $e->getMessage(),
        ]);
    }
